"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.bookTickets = void 0;
var error_handler_1 = require("../utils/error-handler");
var event_model_1 = __importDefault(require("../models/event.model"));
var ticket_model_1 = __importDefault(require("../models/ticket.model"));
var qrcode_1 = __importDefault(require("qrcode"));
var notification_controllers_1 = require("../controllers/notification.controllers");
var bookTickets = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var userId, eventId, ticketCount, event_1, tickets, bookedTickets, totalBooked, newTicket, qrData, qrCodeUrl, error_1, errorMessage;
    var _a;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _b.trys.push([0, 6, , 7]);
                userId = (_a = req.user) === null || _a === void 0 ? void 0 : _a.id;
                if (!userId) {
                    return [2 /*return*/, res.status(401).json({ error: "User not authenticated. Please log in." })];
                }
                eventId = req.params.id;
                ticketCount = req.body.ticketCount;
                return [4 /*yield*/, event_model_1.default.findById(eventId)];
            case 1:
                event_1 = _b.sent();
                if (!event_1) {
                    return [2 /*return*/, res.status(404).json({ error: "Event not found." })];
                }
                return [4 /*yield*/, ticket_model_1.default.find({ eventId: eventId })];
            case 2:
                tickets = _b.sent();
                bookedTickets = tickets.reduce(function (total, ticket) {
                    var ticketCount = ticket.ticketCount || 0;
                    return total + Number(ticketCount);
                }, 0);
                totalBooked = bookedTickets + ticketCount;
                if (event_1.capacity != null && totalBooked > event_1.capacity) {
                    return [2 /*return*/, res.status(400).json({ error: "Not enough capacity available." })];
                }
                return [4 /*yield*/, ticket_model_1.default.create({
                        userId: userId,
                        eventId: eventId,
                        ticketCount: ticketCount,
                    })];
            case 3:
                newTicket = _b.sent();
                qrData = "User ID: ".concat(userId, ", Event ID: ").concat(eventId, ", Ticket Count: ").concat(ticketCount);
                return [4 /*yield*/, qrcode_1.default.toDataURL(qrData)];
            case 4:
                qrCodeUrl = _b.sent();
                return [4 /*yield*/, (0, notification_controllers_1.createNotification)(userId, "Ticket confirmed for event ".concat(eventId, "."), "confirmation")];
            case 5:
                _b.sent();
                return [2 /*return*/, res.status(201).json({
                        message: "Tickets booked successfully.",
                        ticket: newTicket,
                        qrCode: qrCodeUrl, // Include QR code in the response
                    })];
            case 6:
                error_1 = _b.sent();
                errorMessage = (0, error_handler_1.errorHandler)(error_1);
                return [2 /*return*/, res.status(500).json({ error: errorMessage })];
            case 7: return [2 /*return*/];
        }
    });
}); };
exports.bookTickets = bookTickets;
